...
. Create schemas to describe your EDI messages according to your implementation convention (if different from the standard).
. Configure the EDI Module to exchange messages with your trading partner.
...
The X12 Module uses a YAML format called ESL (for EDI Schema Language) to represent EDI schemas.  Basic ESLs define the structure of EDI messages in terms of _structures_ (transaction sets, in X12 terminology), _groups_ (loops, in X12), _segments_, _composites_, and _elements_. ESLs for all transaction sets in X12 0030xx, 0040xx, 0050xx, and 006020 versions are included in the connector. 

If you're using an implementation convention which modifies the base standard, define an _overlay schema_. An overlay schema is a special form of ESL that allows you to modify a base schema, such as an X12 4010 850 schema, with your specific conventions. (You don't need an overlay schema if you're using the structure defined by the standard, but many EDI exchanges modify the base definitions in an implementation convention).
...
After you install the connector and configure your schema customizations (if any), you can start using the connector. Your first step is to create an X12 module configuration.
